...
function radar_visualization(config) {
  ...
  config.legend_offset = config.legend_offset || [
    { x: c400 + 30, y: c400 - 270 },
    { x: -(c400 + 280), y: c400 - 270 },
    { x: -(c400 + 280), y: -(c400 - 80) },
    { x: c400 + 30, y: -(c400 - 80) }
  ]
  config.title_offset = config.title_offset || { x: -(c400 + 280), y: -(c400+40) };
  config.footer_offset = config.footer_offset || { x: -(c400 - 280), y: (c400+30) };
  ...
  const rings = [ { radius: c400-200 }, { radius: c400-100 }, { radius: c400 } ];

  function polar(cartesian) { var x = cartesian.x, y = cartesian.y; return { t: Math.atan2(y, x), r: Math.sqrt(x * x + y * y) } }
  function cartesian(polar) { return { x: polar.r * Math.cos(polar.t), y: polar.r * Math.sin(polar.t) } }
  function bounded_interval(value, min, max) { var low = Math.min(min, max), high = Math.max(min, max); return Math.min(Math.max(value, low), high); }
  function bounded_ring(polar, r_min, r_max) { return { t: polar.t, r: bounded_interval(polar.r, r_min, r_max) } }
  function bounded_box(point, min, max) { return { x: bounded_interval(point.x, min.x, max.x), y: bounded_interval(point.y, min.y, max.y) } }

  function segment(quadrant, ring) {
    var polar_min = { t: quadrants[quadrant].radial_min * Math.PI, r: ring === 0 ? 30 : rings[ring - 1].radius + 10 };
    var polar_max = { t: quadrants[quadrant].radial_max * Math.PI, r: rings[ring].radius - 40 };
    var cartesian_min = { x: 15 * quadrants[quadrant].factor_x, y: 15 * quadrants[quadrant].factor_y };
    var cartesian_max = { x: rings[2].radius * quadrants[quadrant].factor_x, y: rings[2].radius * quadrants[quadrant].factor_y };
    return {
      clipx: function(d) { var c = bounded_box(d, cartesian_min, cartesian_max); var p = bounded_ring(polar(c), polar_min.r + 15, polar_max.r - 15); d.x = cartesian(p).x; return d.x; },
      clipy: function(d) { var c = bounded_box(d, cartesian_min, cartesian_max); var p = bounded_ring(polar(c), polar_min.r + 15, polar_max.r - 15); d.y = cartesian(p).y; return d.y; },
      random: function() { return cartesian({ t: random_between(polar_min.t, polar_max.t), r: normal_between(polar_min.r, polar_max.r) }); }
    }
  }
  ...
  for (let quadrant = 0; quadrant < 4; quadrant++) {
    ...
    for (var ring = 0; ring < 3; ring++) { segmented[quadrant][ring] = []; }
  }
  for (var i = 0; i < config.entries.length; i++) {
    var entry = config.entries[i];
  ...
  for (quadrant of [2,3,1,0]) {
    for (var ring = 0; ring < 3; ring++) {
      ...
      for (var i=0; i<entries.length; i++) { entries[i].id = "" + id++; }
    }
  }
        ...
        legend.selectAll(".legend" + quadrant + ring)
          ...
          .enter()
            ...
            .append("text")
              ...
              .call(wrap_text)
              .each(function(){ previousLegendHeight += d3.select(this).node().getBBox().height; });
